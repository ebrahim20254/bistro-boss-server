...
async function run() {
  try {
    ...
    const reviewsCollection = client.db('bossUser').collection('review');
    const cartsCollection = client.db('bossUser').collection('carts');
    ...
    const verifyToken = (req, res, next) =>{
      console.log('inside verify token', console.log(req.headers));
    ...
    app.post('/users', async(req, res) =>{
      ...
      if(existingUser) {
        return res.send({message: 'user in exit', insertedId: null})
      }
...
app.listen(port, () =>{
    console.log(`bistro bos in running port ${port}`);
})